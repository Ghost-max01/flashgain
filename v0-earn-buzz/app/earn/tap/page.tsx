        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-40 te-fadeIn flex items-center justify-center p-4" onClick={() => setShowAutoFreePopup(false)}>
          <div className="te-slideUp w-full max-w-[420px]" onClick={(e) => e.stopPropagation()}>
            <div className="hh-popup hh-auto-tap-modal">
              ...
              <div className="flex flex-col gap-3">
                <button onClick={() => { setShowAutoFreePopup(false); startAutoPlan("free1h"); }} className="hh-popup-btn hh-popup-btn-confirm">
                  <Zap className="w-4 h-4" />
                  Start FREE 20 mins
                </button>
                <button onClick={() => setShowAutoFreePopup(false)} className="hh-popup-btn">Later</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-40 te-fadeIn flex items-center justify-center p-4" onClick={() => setShowAutoPlans(false)}>
          <div className="te-slideUp w-full max-w-[420px] max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="hh-popup hh-auto-tap-modal">
              ...
              <p className="text-gray-400 text-center text-sm mb-4">Only first-time users get 20 mins FREE. After that it is crossed out.</p>
              <div className="space-y-2.5">
                {AUTO_PLANS.map((p, idx) => {
                  ...
                  return (
                    <button key={p.id} disabled={disabled} onClick={() => startAutoPlan(p.id)} className={`w-full relative rounded-xl border p-4 flex items-center justify-between transition-all ${
                      disabled ? "bg-white/5 border-white/10 opacity-50 cursor-not-allowed" : "bg-gradient-to-r from-emerald-500/10 to-teal-500/10 border-emerald-400/40 hover:border-emerald-400/70 hover:from-emerald-500/20 hover:to-teal-500/20"
                    }`}>
                      <div className="flex items-start gap-3 flex-1 text-left">
                        <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ${
                          disabled ? "bg-gray-600/40 text-gray-500" : "bg-emerald-500/40 text-emerald-300"
                        }`}>{idx+1}</div>
                        <div>
                          <div className={`text-sm font-bold leading-tight ${disabled ? "text-gray-400" : "text-white"}`}>
                            {p.label}
                          ...
                          {isLocked && <div className="text-[10px] text-amber-300/70 mt-1">{lockDays}d {lockHours}h remaining</div>}
                        </div>
                      </div>
                      <div className={`flex-shrink-0 px-3.5 py-2 rounded-full text-xs font-bold ml-3 whitespace-nowrap ${
                        disabled ? "bg-gray-600/40 text-gray-400" : "bg-emerald-500 text-white shadow-lg shadow-emerald-500/50"
                      }`}>
              ...
              <p className="text-xs text-center text-white/40 mt-4 px-2">Auto tap locks the orb (no animation) — balance still rises in real time.</p>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-40 te-fadeIn flex items-center justify-center p-4" onClick={() => setShowAutoReq(false)}>
          <div className="te-slideUp w-full max-w-[420px] max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="hh-popup hh-auto-tap-modal">
              ...
              <p className="text-gray-400 text-center text-sm mb-4">Choose one of 3 options. Referrals use a new tracking link and count to your total.</p>
              <div className="space-y-2.5 mb-5">
                <button 
                  ...
                  className={`w-full relative rounded-xl border p-3 flex items-center justify-between transition-all ${
                ...
                >
                  <div className="flex items-start gap-3 flex-1 text-left">
                    ...
                    <div>
                      <div className="text-sm font-bold text-white leading-tight">{AUTO_REQ_TASK[reqPlan]} Tasks</div>
                      <div className="text-xs text-white/60 mt-1">You've done {getPerPlanDone(reqPlan)}/{AUTO_REQ_TASK[reqPlan]}</div>
                    </div>
                  </div>
                  <span className={`flex-shrink-0 px-3 py-1 rounded-full text-xs font-bold ml-2 whitespace-nowrap ${
                ...
                >
                  <div className="flex items-start gap-3 flex-1 text-left">
                    ...
                    <div>
                      <div className="text-sm font-bold text-white leading-tight">{AUTO_REQ_REF[reqPlan]} Referrals</div>
                      <div className="text-xs text-white/60 mt-1">New tracking link generated</div>
                    </div>
                  </div>
                  <span className={`flex-shrink-0 px-3 py-1 rounded-full text-xs font-bold ml-2 whitespace-nowrap ${
                ...
                >
                  <div className="flex items-start gap-3 flex-1 text-left">
                    ...
                    <div>
                      <div className="text-sm font-bold text-white leading-tight">Pay ₦{AUTO_REQ_PAY[reqPlan].toLocaleString()}</div>
                      <div className="text-xs text-white/60 mt-1">One-time payment unlock</div>
                    </div>
                  </div>
                  <span className={`flex-shrink-0 px-3 py-1 rounded-full text-xs font-bold ml-2 whitespace-nowrap ${
              ...
              <div className="flex gap-3 flex-col">
                <button onClick={fulfillRequirement} disabled={!reqChoice} className={`hh-popup-btn ${
                  reqChoice ? "hh-popup-btn-confirm" : "opacity-50 cursor-not-allowed bg-white/10"
                }`}>Unlock & Start Auto Tap</button>
                <button onClick={() => setShowAutoReq(false)} className="hh-popup-btn">Cancel</button>
              </div>
            </div>
          </div>
        </div>

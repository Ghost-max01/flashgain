...
export default function AutoTapTaskPage() {
  ...
  useEffect(() => {
    ...
    const completed = safeParse(localStorage.getItem("auto-tap-completed-tasks"), [])
    setCompletedTasks(Array.isArray(completed) ? completed : [])

    const savedCooldowns = safeParse(localStorage.getItem("auto-tap-task-cooldowns"), {})
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      if (changed) {
        ...
        localStorage.setItem("auto-tap-task-cooldowns", JSON.stringify(updated))
      }
    }, 1000)
  ...
  const completeVerification = async (taskId: string) => {
    ...
    localStorage.setItem("auto-tap-task-cooldowns", JSON.stringify(newCooldowns))
      ...
      <div className="sticky top-0 z-10 hh-header">
        <div className="max-w-md mx-auto px-6 pt-8 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="hh-title">Auto Tap Tasks</h1>

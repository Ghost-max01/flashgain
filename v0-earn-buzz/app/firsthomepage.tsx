...
export default function FirstHomepage() {
  ...
  const handleInstallClick = async () => {
    // Android with deferredPrompt -> first tap EXPANDS the card (it grows),
    // second tap fires the native install; iOS -> expand card 2x with steps
    if (deferredPrompt) {
    ...
    <div className="min-h-screen bg-[#f6f7fb] text-gray-900 antialiased pb-[88px] md:pb-0">
        ...
        <div className="grid lg:grid-cols-[1.05fr_0.95fr] gap-8 items-start">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-50 border border-emerald-100 text-xs font-semibold text-emerald-700">
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" /> New • Start tapping to earn today
            </div>
      ...
      <section className="max-w-6xl mx-auto px-4 md:px-6 pt-4">
        ...
        <div className="mt-4 grid md:grid-cols-2 lg:grid-cols-4 gap-4">
          {[{ title: "Create account", desc: "Join in 30 seconds with your name, email and phone.", icon: UserPlus, color: "bg-sky-500", num: "01" },{ title: "Tap & earn", desc: "Start tapping and complete simple tasks to grow your balance.", icon: ClipboardCheck, color: "bg-emerald-500", num: "02" },{ title: "Invite & scale", desc: "Share your link — earn ₦500 per successful referral.", icon: Share2, color: "bg-amber-500", num: "03" },{ title: "Cash out fast", desc: "Withdraw to any Nigerian bank, instantly.", icon: Wallet, color: "bg-rose-500", num: "04" }].map((s) => (
      ...
      <section className="max-w-6xl mx-auto px-4 md:px-6 pt-6 grid lg:grid-cols-3 gap-4">
        <div className="bg-white rounded-[24px] border border-gray-100 shadow-sm p-5">
          ...
          <p className="mt-3 text-sm leading-6 text-gray-500">Start at Starter and climb to Legend. Higher levels unlock more daily taps and bigger payouts — stay consistent, earn more.</p>
        ...
        <div className="bg-gradient-to-br from-sky-500 to-indigo-600 rounded-[24px] p-5 text-white shadow-lg shadow-indigo-200 relative overflow-hidden">
          ...
          <div className="relative"><div className="w-10 h-10 rounded-xl bg-white text-sky-600 grid place-items-center"><Users className="h-5 w-5" /></div><h3 className="mt-3 font-black text-white text-[16px]">Invite & earn</h3><p className="mt-2 text-sm leading-6 text-white/80">Share your link and get <b className="text-white">₦500</b> credited instantly per successful referral — plus ongoing referral perks as they earn.</p><Link href="/register" className="mt-4 inline-flex px-4 py-2 rounded-full bg-white text-sky-700 text-sm font-bold">Copy invite link</Link></div>
        </div>
        <div className="bg-white rounded-[24px] border border-gray-100 shadow-sm p-5">
      ...
      <section className="max-w-6xl mx-auto px-4 md:px-6 pt-6">
        ...
        <div className="mt-4 grid md:grid-cols-2 gap-4">
          <div className="bg-white rounded-[24px] border border-gray-100 shadow-sm p-6 relative overflow-hidden"><div className="absolute right-4 bottom-4 opacity-[0.06]"><Users className="h-24 w-24" /></div><div className="relative"><div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-50 border border-emerald-100 text-xs font-bold text-emerald-700"><Sparkles className="h-3.5 w-3.5" /> For earners</div><h3 className="mt-3 font-black text-lg">Turn gaps in your day into cash</h3><p className="mt-2 text-sm leading-6 text-gray-500">Whether you have 1 minute or 1 hour, there is a task that fits — daily taps, tasks, and referrals that compound.</p></div></div>

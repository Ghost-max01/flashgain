...
import { loadMeta, saveMeta, computeScore, getLevel, getProgress, getNextLabel } from "@/lib/trust-score";
...
const AVAILABLE_TASKS_DEDUPED: Task[] = (() => {
  ...
  return AVAILABLE_TASKS.filter((t) => { if (seen.has(t.id)) return false; seen.add(t.id); return true; });
})();
...
export default function TaskPage() {
  ...
  const { attachFocusListener, startTaskTimer, processPendingOnMount } = useTaskTimer()
  ...
  const completeVerification = useCallback(async (taskId: string) => {
    ...
    const nextReset = boundary.getTime()
    setCooldowns((prev) => {
  ...
  const startProgressAnimation = (taskId: string) => {
    ...
    setVerifyingTasks((prev) => ({ ...prev, [taskId]: { progress: 0, startTime } }))
    const interval = setInterval(() => {
      setVerifyingTasks((prev) => {
        ...
        if (newProgress >= 100) { clearInterval(progressIntervals.current[taskId]); delete progressIntervals.current[taskId] }
        return { ...prev, [taskId]: { ...prev[taskId], progress: newProgress } }
      })
    }, 1000)
    ...
    const timer = setInterval(() => {
      ...
      setCooldowns((prev) => {
        ...
        Object.keys(updated).forEach((key) => {
          if (updated[key] > now) { updated[key] -= 1000; changed = true } else { delete updated[key]; changed = true }
        })
        if (changed) {
          if (Object.keys(updated).length === 0) localStorage.removeItem("tivexx-task-cooldowns")
          else localStorage.setItem("tivexx-task-cooldowns", JSON.stringify(updated))
          return updated
        }
        return prev
      })
  ...
  useEffect(() => {
    const detach = attachFocusListener(
      (taskId: string, elapsed: number) => {
        if (!completedTasks.includes(taskId)) { completeVerification(taskId); toast({ title: "Task Completed 🎉", description: "Reward has been added to your balance!" }) }
      },
      (taskId: string, elapsed: number) => {
        setVerifyingTasks((prev) => { const n = { ...prev }; delete n[taskId]; return n })
        if (progressIntervals.current[taskId]) { clearInterval(progressIntervals.current[taskId]); delete progressIntervals.current[taskId] }
        const timeSpent = Math.round(elapsed)
        toast({ title: "You didn't interact with the task ❌", description: `You only spent ${timeSpent}s outside. Please tap the task again and stay on the page for at least ${TASK_VISIT_SECONDS} seconds before coming back.`, variant: "destructive", duration: 6000 })
      },
      (taskId: string) => { return completedTasks.includes(taskId) }
    )
    return detach
  }, [completedTasks, toast, completeVerification, attachFocusListener])

  useEffect(() => {
    if (!loaded) return
    // Process any pending timers from previous visits (handles page reloads)
    const processed = processPendingOnMount(
      (taskId: string) => { if (!completedTasks.includes(taskId)) { completeVerification(taskId); toast({ title: "Task Completed 🎉", description: "Reward has been added to your balance!" }) } },
      (taskId: string, elapsed: number) => {
        setVerifyingTasks((prev) => { const n = { ...prev }; delete n[taskId]; return n })
        if (progressIntervals.current[taskId]) { clearInterval(progressIntervals.current[taskId]); delete progressIntervals.current[taskId] }
        const timeSpent = Math.round(elapsed)
    ...
    if (!processed) {
      // No pending timer; check if any completed task should show as pending/verifying on mount
      // (recovers verifying state if it was lost to navigation/reload)
    }
  }, [loaded, completedTasks, toast, completeVerification, processPendingOnMount])

  const handleTaskClick = (task: Task) => {
    if (completedTasks.includes(task.id)) {
      toast({ title: "Task Already Completed", description: "You have already earned the reward for this task.", variant: "destructive" })
      return
    }
    if (cooldowns[task.id] && cooldowns[task.id] > Date.now()) {
      toast({ title: "Task on Cooldown", description: "This task resets at the next 12-hour interval.", variant: "destructive" })
      return
    }
    if (!task.link) {
      toast({ title: "No link set", description: "This task doesn't have a link yet. Please add one before attempting.", variant: "destructive" })
      return
    }
  ...
  const confirmStartTask = (task: Task) => {
    toast({ title: "Task Started ⏱️", description: `Make sure to spend at least ${TASK_VISIT_SECONDS} seconds on the site before returning. If you return too quickly, you'll need to try again!`, duration: 5000 })
    startProgressAnimation(task.id)
    try { startTaskTimer(task.id) } catch (e) { console.error("Failed to start task timer:", e) }
    // Open in new tab so the app stays available; fallback to same window
    const url = task.link
    try {
    ...
    <div className="hh-root min-h-screen pb-28 relative overflow-hidden">
      ...
      {showCoinRain && (
        <div className="coin-rain">
          {[...Array(30)].map((_, i) => (
            <div key={i} className="coin" style={{ left: `${Math.random() * 100}vw`, animationDelay: `${Math.random() * 2}s` }} />
          ))}
        </div>
      )}
      {/* Header */}
      ...
      <div className="max-w-md mx-auto px-4 space-y-4 pt-2 relative z-10 pb-6">
        ...
        <div className="hh-card hh-card-hero hh-entry-1 relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <p className="text-white/80 text-sm">Complete tasks to earn bonus credits and boost your earnings</p>
              ...
              >
                ...
                <div className="hh-task-body">
                  ...
                  <button
                    ...
                    className={`hh-task-btn ${isCompleted ? 'hh-task-btn-completed' : isPending ? 'hh-task-btn-pending' : isProcessing ? 'hh-task-btn-processing' : 'hh-task-btn-available'}`}
                  >
                  ...
                  {isVerifying && (
                    <div className="mt-3">
                      ...
                      <div className="hh-progress-track">
                        <div className="hh-progress-fill hh-progress-verify" style={{ width: `${progress}%` }}></div>
                      </div>
                    </div>
                  )}
                  {cooldown && timeLeft > 0 && (
        ...
        <div className="hh-card hh-tip-card hh-entry-6">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-emerald-200/80">Tasks reset at 12:00 AM and 12:00 PM every day. Check back after the next interval for more rewards!</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <style jsx global>{`
        ...
        .hh-back-btn { width: 40px; height: 40px; border-radius: 12px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); display: flex; align-items: center; justify-content: center; color: white; transition: all 0.2s ease; cursor: pointer; touch-action: manipulation; -webkit-tap-highlight-color: transparent; }
        .hh-back-btn:hover { background: rgba(255,255,255,0.1); transform: scale(1.05); }
        .hh-back-btn:active { transform: scale(0.95); }
        .hh-title { font-size: 20px; font-weight: 800; color: white; line-height: 1.2; }
        .hh-subtitle { font-size: 12px; color: rgba(16,185,129,0.8); }
        .hh-card { background: linear-gradient(135deg, rgba(255,255,255,0.06) 0%, rgba(255,255,255,0.02) 100%); border: 1px solid rgba(255,255,255,0.08); border-radius: 20px; padding: 20px; backdrop-filter: blur(12px); position: relative; overflow: hidden; transition: transform 0.25s ease, box-shadow 0.25s ease; }
        .hh-card:hover { transform: translateY(-2px); box-shadow: 0 20px 60px rgba(0,0,0,0.4), 0 0 30px rgba(16,185,129,0.05); }
        ...
        .hh-orb-1 { width: 150px; height: 150px; background: radial-gradient(circle, rgba(16,185,129,0.2), transparent); top: -40px; right: -40px; animation: hh-orb-float 6s ease-in-out infinite; }
        .hh-orb-2 { width: 100px; height: 100px; background: radial-gradient(circle, rgba(245,158,11,0.15), transparent); bottom: 20px; left: -20px; animation: hh-orb-float 8s ease-in-out infinite reverse; }
        ...
        .hh-stats-row { display: flex; align-items: center; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.06); border-radius: 12px; padding: 12px 16px; }
        ...
        .hh-task-card { background: linear-gradient(135deg, rgba(255,255,255,0.05) 0%, rgba(255,255,255,0.02) 100%); border: 1px solid rgba(255,255,255,0.08); border-radius: 20px; overflow: hidden; transition: all 0.3s ease; animation: hh-card-appear 0.4s ease-out both; }
        .hh-task-card:hover { transform: translateY(-2px); border-color: rgba(16,185,129,0.3); box-shadow: 0 10px 30px rgba(0,0,0,0.3); }
        .hh-task-header { padding: 16px; padding-right: 48px; border-bottom: 1px solid rgba(255,255,255,0.05); position: relative; }
        .hh-task-number { position: absolute; top: 14px; right: 16px; font-family: 'JetBrains Mono', monospace; font-size: 11px; font-weight: 800; color: rgba(255, 255, 255, 0.4); background: rgba(255, 255, 255, 0.03); border: 1px solid rgba(255, 255, 255, 0.08); padding: 2px 8px; border-radius: 6px; pointer-events: none; transition: all 0.3s ease; }
        .hh-task-card:hover .hh-task-number { color: #10b981; border-color: rgba(16, 185, 129, 0.3); background: rgba(16, 185, 129, 0.08); box-shadow: 0 0 10px rgba(16, 185, 129, 0.15); }
        .hh-task-icon { width: 48px; height: 48px; border-radius: 14px; background: linear-gradient(135deg, rgba(16,185,129,0.1), rgba(245,158,11,0.1)); border: 1px solid rgba(16,185,129,0.2); display: flex; align-items: center; justify-content: center; }
        .hh-task-title { font-weight: 700; color: white; font-size: 15px; margin-bottom: 4px; }
        .hh-task-desc { font-size: 12px; color: rgba(255,255,255,0.5); }
        .hh-task-body { padding: 16px; }
        .hh-reward-badge { display: inline-flex; align-items: center; gap: 4px; background: linear-gradient(135deg, rgba(245,158,11,0.2), rgba(245,158,11,0.1)); border: 1px solid rgba(245,158,11,0.3); border-radius: 20px; padding: 4px 10px; font-size: 13px; font-weight: 700; color: #fbbf24; }
        .hh-status-badge { display: inline-flex; align-items: center; gap: 4px; padding: 4px 10px; border-radius: 20px; font-size: 11px; font-weight: 700; }
        .hh-status-completed { background: rgba(16,185,129,0.15); border: 1px solid rgba(16,185,129,0.3); color: #10b981; }
        .hh-status-pending { background: rgba(245,158,11,0.15); border: 1px solid rgba(245,158,11,0.3); color: #fbbf24; }
        .hh-task-btn { width: 100%; padding: 14px; border-radius: 14px; font-weight: 700; font-size: 14px; border: none; cursor: pointer; transition: all 0.2s ease; color: white; touch-action: manipulation; -webkit-tap-highlight-color: transparent; }
        .hh-task-btn:hover { transform: translateY(-2px); }
        .hh-task-btn:active { transform: scale(0.98); }
        .hh-task-btn-available { background: linear-gradient(135deg, #10b981, #059669); box-shadow: 0 4px 15px rgba(16,185,129,0.3); }
        .hh-task-btn-pending { background: linear-gradient(135deg, #fbbf24, #d97706); box-shadow: 0 4px 15px rgba(245,158,11,0.3); }
        .hh-task-btn-completed { background: rgba(255,255,255,0.1); cursor: not-allowed; color: rgba(255,255,255,0.4); }
        .hh-task-btn-processing { background: rgba(59,130,246,0.3); cursor: not-allowed; border: 1px solid rgba(59,130,246,0.3); }
        .hh-progress-track { width: 100%; height: 6px; background: rgba(255,255,255,0.08); border-radius: 10px; overflow: hidden; }
        .hh-progress-fill { height: 100%; border-radius: 10px; transition: width 0.3s ease; }
        .hh-progress-verify { background: linear-gradient(90deg, #fbbf24, #d97706); box-shadow: 0 0 10px rgba(245,158,11,0.5); }
        .hh-cooldown { display: flex; align-items: center; justify-content: center; gap: 8px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05); border-radius: 30px; padding: 8px 12px; font-size: 12px; color: #fbbf24; }
        .hh-task-warning { background: rgba(245,158,11,0.1); border-top: 1px solid rgba(245,158,11,0.2); padding: 12px 16px; font-size: 11px; color: rgba(255,255,255,0.7); display: flex; align-items: center; gap: 6px; }
        ...
        .coin-rain { position: fixed; top: 0; left: 0; width: 100%; height: 100%; pointer-events: none; z-index: 1000; }
        .coin { position: absolute; width: 24px; height: 24px; background: linear-gradient(135deg, #fbbf24, #d97706); border-radius: 50%; animation: coin-fall 3s linear forwards; box-shadow: 0 4px 8px rgba(0,0,0,0.3); top: -30px; }
        @keyframes coin-fall { 0% { transform: translateY(0) rotate(0deg); opacity: 1; } 100% { transform: translateY(100vh) rotate(720deg); opacity: 0; } }
        ...
        .hh-nav-item { display: flex; flex-direction: column; align-items: center; gap: 3px; color: #4b5563; text-decoration: none; font-size: 11px; font-weight: 600; transition: color 0.2s, transform 0.2s; padding: 8px 16px; border-radius: 12px; touch-action: manipulation; -webkit-tap-highlight-color: transparent; }
        ...
        @keyframes hh-card-appear { from { opacity: 0; transform: translateY(12px) scale(0.97); } to { opacity: 1; transform: translateY(0) scale(1); } }
        ...
        @media (prefers-reduced-motion: reduce) { .hh-bubble, .hh-orb-1, .hh-orb-2, .hh-orb-3, .coin, [class*="hh-entry-"] { animation: none !important; } }
        .hh-modal-backdrop { position: fixed; inset: 0; background: rgba(0,0,0,0.6); display: flex; align-items: center; justify-content: center; z-index: 220; padding: 24px; }
        .hh-modal { width: 100%; max-width: 440px; background: linear-gradient(135deg, rgba(255,255,255,0.03), rgba(255,255,255,0.01)); border: 1px solid rgba(255,255,255,0.06); border-radius: 16px; padding: 18px; backdrop-filter: blur(8px); }
        .hh-modal-title { font-weight: 800; color: white; margin-bottom: 8px; }
        .hh-modal-desc { color: rgba(255,255,255,0.9); font-size: 14px; line-height: 1.35; }
        .hh-modal-actions { display: flex; justify-content: flex-end; gap: 10px; margin-top: 14px; }
        .hh-modal-btn { padding: 10px 14px; border-radius: 10px; font-weight: 700; cursor: pointer; }
        .hh-modal-cancel { background: transparent; border: 1px solid rgba(255,255,255,0.06); color: white; }
        .hh-modal-continue { background: linear-gradient(135deg, #10b981, #059669); color: white; border: none; }
      `}</style>
    </div>
  )
}
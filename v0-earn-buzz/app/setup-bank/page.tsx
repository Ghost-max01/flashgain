...
export default function SetupBankAfterSignupPage() {
  ...
  // Redirect guard: must be logged in (tivexx-user exists), otherwise send to register
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (mounted && data && data.banks) {
          const filteredList = data.banks.filter((bankItem: any) => {
            const n = bankItem.name.toLowerCase()
            return (
  ...
  async function verifyAccount() {
    ...
    setVerifying(true)
    try {
      console.log("setup-bank.verifyAccount request", { account_number: accountNumber.replace(/\D/g, ""), bank_code: bankCode })
      ...
      const data = await res.json()
      console.log("setup-bank.verifyAccount response", { status: res.status, body: data })
  ...
  useEffect(() => {
    ...
    if (cleaned.length === 10 && bankCode && !isLocked) {
      const t = setTimeout(() => {
        verifyAccount()
      }, 350)
    ...
    return (
      <div className="hh-root min-h-screen flex items-center justify-center relative overflow-hidden">
        ...
        <style jsx global>{`
          ...
          .hh-loading-container { position: relative; z-index: 10; text-align: center; animation: hh-entry 0.5s ease-out; }
          .hh-loading-spinner { position: relative; width: 80px; height: 80px; margin: 0 auto 24px; }
          .hh-spinner-ring { position: absolute; width: 100%; height: 100%; border-radius: 50%; border: 3px solid transparent; border-top-color: #10b981; animation: hh-spin 1.5s cubic-bezier(0.68,-0.55,0.265,1.55) infinite; }
          .hh-spinner-ring-2 { width: 70%; height: 70%; top: 15%; left: 15%; border-top-color: #fbbf24; animation-duration: 2s; animation-direction: reverse; }
          .hh-spinner-ring-3 { width: 40%; height: 40%; top: 30%; left: 30%; border-top-color: #3b82f6; animation-duration: 1.2s; }
          @keyframes hh-spin { 0% { transform: rotate(0deg);}100%{transform:rotate(360deg)}}
          .hh-loading-title { font-size: 28px; font-weight: 800; background: linear-gradient(135deg, #10b981, #fbbf24); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin-bottom: 8px; animation: hh-glow 2.5s infinite alternate; }
          @keyframes hh-glow {0%{text-shadow:0 0 5px rgba(16,185,129,0.3)}100%{text-shadow:0 0 20px rgba(16,185,129,0.6),0 0 30px rgba(251,191,36,0.3)}}
          .hh-loading-text { color: rgba(255,255,255,0.7); font-size: 14px; }
  ...
  return (
    <div className="hh-root min-h-screen pb-28 relative overflow-hidden">
        ...
        <div className="hh-card hh-entry-2">
          <div className="space-y-5">
            ...
            <div ref={dropdownRef} className="relative">
                ...
                <div className="hh-dropdown">
                  <div className="hh-dropdown-search">
                    <div className="hh-search-container">
                      ...
                      />
                      {bankSearchInput && (
                        <button onClick={() => setBankSearchInput("")} className="hh-search-clear">
                          <X className="h-3 w-3" />
                        </button>
                      )}
                    </div>
                    <div className="hh-search-stats">
                      ...
                      <span className="flex items-center gap-1"><Sparkles className="h-3 w-3" /> Quick search!</span>
                    </div>
                  </div>
                    ...
                    {banksList.length > 0 ? (
                      filteredBanks.length > 0 ? (
                        filteredBanks.map((bankItem) => (
                          <div key={bankItem.code} onClick={() => handleBankSelect(bankItem.name, bankItem.code)} className={`hh-bank-item ${bank === bankItem.name ? 'hh-bank-item-selected' : ''}`}>
                        ...
                        <div className="hh-empty-state">
                          <div className="hh-empty-icon"><Search className="h-6 w-6" /></div>
              ...
              {bank && (
                <p className="hh-selected-bank">Selected: <span className="font-medium">{bank}</span> {isLocked && " 🔒"}</p>
              )}
            </div>
            ...
            <div>
              ...
              <div className="hh-input-group">
                ...
                <button
                  ...
                  className={`hh-verify-btn ${isLocked || accountNumber.replace(/\D/g, "").length !== 10 || !bankCode ? 'hh-verify-disabled' : 'hh-verify-active'}`}
                >
                  {verifying ? (<span className="hh-verify-loading"><span className="hh-spinner-small"></span>Verifying</span>) : ("Verify")}
                </button>
              </div>
            ...
            <div className="hh-entry-3">
              ...
              <input
                value={accountName}
                onChange={(e) => { if (!verified && !isLocked) setAccountName(e.target.value) }}
            ...
            <button
              ...
              className={`hh-proceed-btn ${(!isLocked && (!bank || !accountNumber || !accountName)) ? 'hh-proceed-disabled' : 'hh-proceed-active'} hh-entry-4`}
            >
      ...
      <style jsx global>{`
        ...
        .hh-label { display: block; font-size: 13px; font-weight: 600; color: #10b981; margin-bottom: 8px; letter-spacing: 0.02em; }
        .hh-select-btn { width: 100%; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); border-radius: 14px; padding: 14px 16px; display: flex; align-items: center; justify-content: space-between; color: white; transition: all 0.2s ease; cursor: pointer; }
        .hh-select-btn:hover { background: rgba(255,255,255,0.08); border-color: rgba(16,185,129,0.3); }
        .hh-select-arrow { width: 18px; height: 18px; color: #10b981; transition: transform 0.3s ease; }
        .hh-dropdown { position: absolute; z-index: 50; margin-top: 8px; width: 100%; background: linear-gradient(135deg, #0d1f2d, #0a1628); border: 1px solid rgba(255,255,255,0.1); border-radius: 16px; overflow: hidden; box-shadow: 0 20px 40px rgba(0,0,0,0.4); animation: hh-dropdown-appear 0.3s cubic-bezier(0.34,1.56,0.64,1); }
        @keyframes hh-dropdown-appear { from{opacity:0;transform:translateY(-10px) scale(0.95)} to{opacity:1;transform:translateY(0) scale(1)}}
        .hh-dropdown-search { background: linear-gradient(135deg, #0a1a24, #051016); padding: 16px; border-bottom: 1px solid rgba(16,185,129,0.2); }
        .hh-search-container { position: relative; margin-bottom: 8px; }
        .hh-search-icon { position: absolute; left: 12px; top: 50%; transform: translateY(-50%); width: 16px; height: 16px; color: #10b981; }
        .hh-sparkle-icon { position: absolute; left: 32px; top: 50%; transform: translateY(-50%); width: 12px; height: 12px; color: #fbbf24; animation: hh-sparkle-pulse 1.5s ease-in-out infinite; }
        @keyframes hh-sparkle-pulse {0%,100%{opacity:0.5;transform:translateY(-50%) scale(1)}50%{opacity:1;transform:translateY(-50%) scale(1.2)}}
        .hh-search-input { width: 100%; padding: 12px 16px 12px 48px; background: rgba(255,255,255,0.05); border: 1px solid rgba(16,185,129,0.3); border-radius: 30px; color: white; font-size: 14px; transition: all 0.2s ease; }
        .hh-search-input:focus { outline: none; border-color: #10b981; box-shadow: 0 0 0 3px rgba(16,185,129,0.2); }
        .hh-search-input::placeholder { color: rgba(255,255,255,0.3); }
        .hh-search-clear { position: absolute; right: 12px; top: 50%; transform: translateY(-50%); width: 20px; height: 20px; border-radius: 50%; background: rgba(16,185,129,0.2); border: 1px solid rgba(16,185,129,0.3); display: flex; align-items: center; justify-content: center; color: #10b981; transition: all 0.2s ease; }
        .hh-search-clear:hover { background: rgba(16,185,129,0.3); transform: translateY(-50%) scale(1.1); }
        .hh-search-stats { display: flex; align-items: center; justify-content: space-between; font-size: 11px; color: rgba(255,255,255,0.5); }
        .hh-bank-list { max-height: 300px; overflow-y: auto; padding: 8px; }
        .hh-bank-item { padding: 12px 16px; margin: 2px 0; border-radius: 12px; display: flex; align-items: center; gap: 12px; cursor: pointer; transition: all 0.2s ease; color: rgba(255,255,255,0.8); }
        .hh-bank-item:hover { background: linear-gradient(135deg, rgba(16,185,129,0.15), rgba(16,185,129,0.05)); transform: translateX(4px); }
        .hh-bank-item-selected { background: rgba(16,185,129,0.1); border: 1px solid rgba(16,185,129,0.3); }
        .hh-bank-indicator { width: 6px; height: 6px; border-radius: 50%; background: #10b981; transition: all 0.2s ease; }
        .hh-bank-item:hover .hh-bank-indicator { background: #fbbf24; transform: scale(1.2); }
        .hh-empty-state { text-align: center; padding: 32px 16px; }
        .hh-empty-icon { width: 48px; height: 48px; border-radius: 50%; background: rgba(16,185,129,0.1); border: 1px solid rgba(16,185,129,0.2); display: flex; align-items: center; justify-content: center; margin: 0 auto 12px; color: #10b981; }
        .hh-empty-title { font-weight: 600; color: white; margin-bottom: 4px; }
        .hh-empty-desc { font-size: 12px; color: rgba(255,255,255,0.4); }
        .hh-loading-banks { text-align: center; padding: 32px 16px; color: rgba(255,255,255,0.5); }
        .hh-loading-spinner-small { width: 24px; height: 24px; border: 2px solid rgba(16,185,129,0.3); border-top-color: #10b981; border-radius: 50%; animation: hh-spin 1s linear infinite; margin: 0 auto 12px; }
        .hh-selected-bank { font-size: 12px; color: #10b981; margin-top: 6px; }
        .hh-locked-banner { display:flex; align-items:center; gap:8px; background: rgba(251,191,36,0.12); border:1px solid rgba(251,191,36,0.3); border-radius:12px; padding:10px 12px; color:#fbbf24; font-size:12px;}
        .hh-input-group { display: flex; gap: 8px; }
        .hh-input { background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); border-radius: 14px; padding: 14px 16px; color: white; font-size: 15px; transition: all 0.2s ease; }
        .hh-input:focus { outline: none; border-color: #10b981; box-shadow: 0 0 0 3px rgba(16,185,129,0.1); }
        .hh-input::placeholder { color: rgba(255,255,255,0.3); }
        .hh-input-verified { background: rgba(16,185,129,0.1); border-color: rgba(16,185,129,0.3); color: #10b981; }
        .hh-verify-btn { padding: 0 20px; border-radius: 14px; font-weight: 600; font-size: 14px; border: none; cursor: pointer; transition: all 0.2s ease; white-space: nowrap; }
        .hh-verify-active { background: linear-gradient(135deg, #10b981, #059669); color: white; box-shadow: 0 4px 15px rgba(16,185,129,0.3); }
        .hh-verify-active:hover { transform: translateY(-2px); box-shadow: 0 8px 25px rgba(16,185,129,0.5); }
        .hh-verify-active:active { transform: scale(0.97); }
        .hh-verify-disabled { background: rgba(255,255,255,0.05); color: rgba(255,255,255,0.3); cursor: not-allowed; border: 1px solid rgba(255,255,255,0.1); }
        .hh-verify-loading { display: flex; align-items: center; gap: 8px; }
        .hh-spinner-small { width: 16px; height: 16px; border: 2px solid rgba(255,255,255,0.3); border-top-color: white; border-radius: 50%; animation: hh-spin 1s linear infinite; }
        .hh-error-message { color: #f87171; font-size: 13px; margin-top: 6px; padding-left: 4px; }
        .hh-verified-badge { margin-left: 10px; background: rgba(16,185,129,0.15); border: 1px solid rgba(16,185,129,0.3); color: #10b981; font-size: 11px; padding: 2px 8px; border-radius: 20px; font-weight: 500; }
        .hh-verified-note { font-size: 11px; color: #10b981; margin-top: 4px; }
        .hh-proceed-btn { width: 100%; padding: 18px; border-radius: 16px; font-weight: 800; font-size: 16px; border: none; cursor: pointer; transition: all 0.3s ease; margin-top: 8px; }
        .hh-proceed-active { background: linear-gradient(135deg, #10b981, #059669, #047857); color: white; box-shadow: 0 6px 30px rgba(16,185,129,0.4); animation: hh-btn-glow 2s ease-in-out infinite; }
        .hh-proceed-active:hover { transform: translateY(-2px); box-shadow: 0 10px 40px rgba(16,185,129,0.6); }
        .hh-proceed-active:active { transform: scale(0.98); }
        .hh-proceed-disabled { background: rgba(255,255,255,0.05); color: rgba(255,255,255,0.3); cursor: not-allowed; border: 1px solid rgba(255,255,255,0.1); }
        ...
        @media (prefers-reduced-motion: reduce) {.hh-bubble,.hh-orb-1,.hh-orb-2,.hh-proceed-active,.hh-sparkle-icon,[class*="hh-entry-"]{animation:none !important}}
      `}</style>
    </div>
  )
}

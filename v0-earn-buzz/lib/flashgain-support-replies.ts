...
// Shared Moneymate Support automated replies — single source so the
...
export function getMoneymateSupportReply(input: string): SupportReply & { followUpMenu: string | null } {
  ...
  if (["about", "Moneymate", "company", "what is"].some((w) => clean.includes(w))) {

    ...
    <div className="min-h-screen bg-[#f6f7fb] text-gray-900 antialiased pb-[88px] md:pb-0">
      ...
      <header className="sticky top-0 z-40 backdrop-blur-xl bg-white/70 border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-4 md:px-6 h-[64px] flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Link
              href="/login"
              className="hidden md:inline-flex px-5 py-2.5 rounded-full bg-white border border-gray-200 text-sm font-semibold hover:bg-gray-50 transition"
            >
              Sign in
            </Link>
            <Link
              href="/login"
              className="md:hidden px-4 py-2 rounded-full bg-white border border-gray-200 text-sm font-semibold"
            >
              Sign in
            </Link>
            <Link
              href="/register"
              className="hidden md:inline-flex px-5 py-2.5 rounded-full bg-gray-900 text-white text-sm font-bold hover:bg-black transition"
            >
              Get started
            </Link>
          </div>
        ...
        <div className="grid lg:grid-cols-[1.05fr_0.95fr] gap-8 items-start">
          <div>
            ...
            <h1 className="mt-5 text-[32px] md:text-[46px] font-black tracking-tight leading-[0.95]">
              <span className="text-gray-900">Turn spare time into</span>
              <br />
            ...
            <div className="mt-7 flex flex-col gap-3 max-w-md">
              <Link
                href="/register"
                className="inline-flex items-center justify-center gap-2 w-full md:w-auto px-8 py-4 rounded-2xl bg-gradient-to-br from-emerald-500 to-emerald-600 text-white font-extrabold text-[15px] shadow-xl shadow-emerald-200 hover:shadow-emerald-300 hover:translate-y-[-1px] transition-all"
              >
                <Rocket className="h-5 w-5" /> Get started — it&apos;s free
              </Link>
              <Link
                href="/login"
                className="inline-flex items-center justify-center gap-2 w-full md:w-auto px-8 py-4 rounded-2xl bg-white border border-gray-200 font-bold text-[15px] hover:bg-gray-50 transition shadow-sm"
              >
            ...
            <div className="mt-6 grid grid-cols-3 gap-3 max-w-md">
              {[
                { k: "48K+", l: "Active earners", c: "text-emerald-600" },
                { k: "2.1M+", l: "Tasks done", c: "text-indigo-600" },
                { k: "₦520M+", l: "Paid out", c: "text-amber-600" },
              ].map((s) => (
          ...
          <div className="relative lg:pl-6">
              ...
              <div className="relative">
                ...
                <div className="mt-5 grid grid-cols-2 gap-3">
                  <div className="rounded-2xl bg-[#f6f7fb] border border-gray-100 p-3">
                    <div className="w-8 h-8 rounded-xl bg-white border border-gray-100 grid place-items-center">
            ...
            <div className="hidden md:flex absolute -bottom-4 -left-2 bg-white rounded-2xl border border-gray-100 shadow-lg p-3 items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-emerald-500 grid place-items-center text-white">
                <TrendingUp className="h-5 w-5" />
              </div>
              <div>
                <div className="text-sm font-black">+₦14,000</div>
                <div className="text-xs text-gray-500">withdrawn today by Tolu • 2m ago</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="max-w-6xl mx-auto px-4 md:px-6 pt-4">
        <div className="flex items-end justify-between gap-4">
          <h2 className="text-xl md:text-2xl font-black tracking-tight">How it works</h2>
          <span className="text-xs font-semibold text-gray-400">4 simple steps</span>
        </div>
          ...
          {[
            {
              ...
              desc: "Join in 30 seconds with your name, email and phone.",
              icon: UserPlus,
              color: "bg-sky-500",
              num: "01",
            },
            ...
            {
              title: "Cash out fast",
              desc: "Withdraw to any Nigerian bank, instantly.",
              icon: Wallet,
              color: "bg-rose-500",
              num: "04",
            },
            ...
            <div
              key={s.title}
              className="relative bg-white rounded-[24px] border border-gray-100 shadow-sm hover:shadow-md transition p-5 flex flex-col min-h-[160px]"
            >
              <div className="flex items-start justify-between">
                <div className={`w-12 h-12 rounded-2xl ${s.color} grid place-items-center text-white shadow-md`}>
                  <s.icon className="h-6 w-6" />
                </div>
                <span className="text-sm font-black text-gray-200">{s.num}</span>
              </div>
              <div className="mt-4 font-bold text-[15px]">{s.title}</div>
              <div className="mt-1 text-sm leading-6 text-gray-500">{s.desc}</div>
            </div>
          ))}
        </div>
      </section>

      {/* TIERS / INVITE / WITHDRAWAL */}
      <section className="max-w-6xl mx-auto px-4 md:px-6 pt-6 grid lg:grid-cols-3 gap-4">
        <div className="bg-white rounded-[24px] border border-gray-100 shadow-sm p-5">
          ...
          <div className="mt-4 flex items-center gap-2 flex-wrap">
            {["Starter", "Bronze", "Silver", "Gold", "Legend"].map((l, i) => (
              <span
                key={l}
                className={`px-2.5 py-1 rounded-full text-xs font-bold border ${i === 4 ? "bg-gray-900 text-white border-gray-900" : "bg-gray-50 border-gray-200 text-gray-700"}`}
              >
                {l}
              </span>
            ))}
          ...
          <div className="mt-4 inline-flex items-center gap-2 text-xs font-semibold text-emerald-600">
            <CircleCheck className="h-4 w-4" /> +1 level per day you complete tasks
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-sky-500 to-indigo-600 rounded-[24px] p-5 text-white shadow-lg shadow-indigo-200 relative overflow-hidden">
          ...
          <div className="relative">
            <div className="w-10 h-10 rounded-xl bg-white text-sky-600 grid place-items-center">
        ...
        <div className="bg-white rounded-[24px] border border-gray-100 shadow-sm p-5">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-amber-400 grid place-items-center text-gray-900">
              <Landmark className="h-5 w-5" />
            </div>
            <h3 className="font-black">Fast withdrawals</h3>
          </div>
          <p className="mt-3 text-sm leading-6 text-gray-500">
            Cash out to any verified Nigerian bank account. Choose flexible amount — your earnings, your rules. No delays, no stories.
          </p>
          <div className="mt-4 flex items-center gap-2 text-xs font-semibold text-gray-700">
            <Clock3 className="h-4 w-4 text-gray-400" /> Processed in minutes
      ...
      <section className="max-w-6xl mx-auto px-4 md:px-6 pt-6">
        ...
        <div className="mt-4 grid md:grid-cols-2 gap-4">
          <div className="bg-white rounded-[24px] border border-gray-100 shadow-sm p-6 relative overflow-hidden">
            ...
            <div className="relative">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-50 border border-emerald-100 text-xs font-bold text-emerald-700">
                <Sparkles className="h-3.5 w-3.5" /> For earners
              </div>
          ...
          <div className="bg-white rounded-[24px] border border-gray-100 shadow-sm p-6 relative overflow-hidden">
            ...
            <div className="relative">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-50 border border-indigo-100 text-xs font-bold text-indigo-700">
                <BadgeCheck className="h-3.5 w-3.5" /> For the ambitious
              </div>
              <h3 className="mt-3 font-black text-lg">Grow faster by growing others</h3>
              <p className="mt-2 text-sm leading-6 text-gray-500">
                Build a referral network. Help friends earn and unlock commission-style rewards as your community grows.
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="max-w-6xl mx-auto px-4 md:px-6 pt-6 pb-8">
        <div className="rounded-[28px] bg-gradient-to-br from-emerald-600 via-indigo-600 to-sky-600 p-[1px] shadow-xl shadow-indigo-200">
          <div className="rounded-[27px] bg-gradient-to-br from-emerald-600 via-indigo-600 to-sky-600 p-6 md:p-8 text-center text-white relative overflow-hidden">
            ...
            <div className="relative">
              ...
              <Link
                href="/register"
                className="mt-6 inline-flex items-center justify-center px-8 py-4 rounded-2xl bg-white text-indigo-700 font-black shadow-lg hover:bg-gray-50 transition"
              >
                Create free account
              </Link>
              <div className="mt-3 text-xs text-white/70">No card required • Takes 30 seconds</div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 flex flex-col md:flex-row items-center justify-between gap-3 text-xs text-gray-400">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link href="/policies?tab=payment" className="hover:text-gray-600">
              Payment Policy
            </Link>
            <Link href="/policies?tab=support" className="hover:text-gray-600">
              Support
            </Link>
          </div>
        </div>
      ...
      {showInstall && (
        <div className="fixed bottom-[92px] md:bottom-4 left-1/2 -translate-x-1/2 w-[95%] max-w-lg z-50">
          <div className="bg-white rounded-[20px] border border-gray-100 shadow-2xl p-3 flex items-center gap-3">
            ...
            <Link
              href="/register"
              className="px-5 py-2.5 rounded-full bg-sky-500 text-white text-sm font-bold hover:bg-sky-600 transition flex-shrink-0"
            >
              Install
            </Link>
            <button onClick={() => setShowInstall(false)} className="w-8 h-8 grid place-items-center rounded-full hover:bg-gray-50 text-gray-400">
              <X className="h-4 w-4" />
            </button>
          </div>
        </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-gray-200 shadow-[0_-4px_24px_rgba(0,0,0,0.06)]">
        <div className="grid grid-cols-5 py-2 safe-pb">
          {[
            ...
            { label: "About", icon: BadgeCheck },
            { label: "Account", icon: Users },
          ].map((it) => (
            <Link
              key={it.label}
              href={it.active ? "/" : it.label === "Tasks" ? "/task" : it.label === "Rewards" ? "/refer" : it.label === "Account" ? "/login" : "/about"}
